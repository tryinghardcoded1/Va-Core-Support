...
try { db.exec("ALTER TABLE users ADD COLUMN subscription_status TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE users ADD COLUMN email_verified BOOLEAN DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE users ADD COLUMN last_login DATETIME;"); } catch(e) {}
try { db.exec("ALTER TABLE users ADD COLUMN admin_notes TEXT;"); } catch(e) {}
...
if (permsCount.count === 0) {
  const insertPerm = db.prepare('INSERT INTO role_permissions (role_name, permission_name) VALUES (?, ?)');
  const adminPerms = ['can_manage_users', 'can_manage_jobs', 'can_manage_payments', 'can_manage_reports', 'can_manage_settings'];
  for (const p of adminPerms) {
    insertPerm.run('admin', p);
  }
}
